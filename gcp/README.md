...
1. **Configure GCP Cloud Integration**
   - [ ] [Enable billing data export](https://www.ibm.com/docs/en/kubecost/self-hosted/3.x?topic=integrations-gcp-cloud-billing-integration#ariaid-title2)
   - [ ] [Create a GCP service account](https://www.ibm.com/docs/en/kubecost/self-hosted/3.x?topic=integrations-gcp-cloud-billing-integration#ariaid-title3)
   - [ ] [Connect using Workload Identity Federation](https://www.ibm.com/docs/en/kubecost/self-hosted/3.x?topic=integrations-gcp-cloud-billing-integration#ariaid-title5)


2. **Configure Storage for Cluster Metrics**
   - [ ] [Set up object storage](https://www.ibm.com/docs/en/kubecost/self-hosted/3.x?topic=configuration-gcp-long-term-storage)
...
```bash
   kubectl create secret generic federated-store --from-file=federated-store.yaml --namespace kubecost
```
...
```bash
helm upgrade --install kubecost --repo https://kubecost.github.io/kubecost/ kubecost --namespace kubecost --version <version> \
